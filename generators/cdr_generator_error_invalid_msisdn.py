...
from cdr_error_modifiers import create_cdr_with_invalid_msisdn
...
OUTPUT_FILENAME = f"generated_cdrs_error_invalid_msisdn.jsonl"
...
def main():
    ...
    while len(generated_error_cdr_records) < NUMBER_OF_CDRS_TO_GENERATE:
        ...
        error_cdr = create_cdr_with_invalid_msisdn(base_call_data)
    ...
    with open(f"out_data/{OUTPUT_FILENAME}", 'w') as f:
        for record in final_records_to_write:
            ...
            f.write('\n')
    print(
        f"Генерация завершена. {len(final_records_to_write)} CDR записей с невалидными номерами сохранено в файл {OUTPUT_FILENAME}")
