...
from cdr_error_modifiers import create_cdr_with_start_after_end
...
OUTPUT_FILENAME = f"generated_cdrs_error_start_after_end.jsonl"
...
def main():
    ...
    while len(generated_error_cdr_records) < NUMBER_OF_CDRS_TO_GENERATE:
        ...
        error_cdr = create_cdr_with_start_after_end(base_call_data)
    ...
    print(
        f"Генерация завершена. {len(generated_error_cdr_records)} CDR записей с ошибкой 'callStart > callEnd' сохранено в файл {OUTPUT_FILENAME}")
